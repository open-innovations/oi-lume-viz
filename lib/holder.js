...
export function VisualisationHolder(config,attr){
	...
	this.wrap = function(inner){
		...
		var deps = ['/css/holder.css'];
		deps = deps.concat(dependencies);
		if(legend.exists()) deps = deps.concat(legend.getDependencies());

		var html = ['<div class="oi-viz'+(classes ? ' '+classes.join(" ") : '')+'" data-dependencies="'+makeDependencies(deps)+'">'];
