...
export function getTimeTicks(config){
	...
	let div = 1;
	let fract = config.tick.spacing;
	
	if(config.tick.type){
		for(let s = 0 ; s < steps.length; s++){
			if(steps[s].name==config.tick.type){
				div = steps[s].div;
				step = steps[s];
				// If we haven't got a fract value we'll work one out
				if(!fract){
					for(let sp = 0; sp < step.spacings.length; sp++){
						let n = Math.ceil(Num(range).div(Num(step.div).times(1000).times(step.spacings[sp])).toValue());
						if(n < 1 || n > 20) continue;
						else{
							if(n >= 3 && n < t_div){
								t_div = n;
								fract = step.spacings[sp];
							}
						}
					}
				}
		...
		for(let st = 0; st < steps.length ; st++){
			for(let sp = 0; sp < steps[st].spacings.length; sp++){
				let n = Math.ceil(Num(range).div(Num(steps[st].div).times(1000).times(steps[st].spacings[sp])).toValue());
				if(n < 1 || n > 20) continue;
				else{
					if(n >= 3 && n < t_div){
						t_div = n;
	...
	let t_inc = Num(div).times(1000).times(fract);

	// Need to round min/max to nearest config.tick.spacing
	let mn = roundDate(config.min,{'range':range,'unit':config.tick.type,'method':'floor'});
	let mx = roundDate(config.max,{'range':range,'unit':config.tick.type,'method':'ceil'});

	let vmx = mx.plus(t_inc.times(0.2)).toValue();
	for(let v = mn, i = 0; v.toValue() <= vmx; v = v.plus(t_inc), i++){
		let d = roundDate(v.toValue(),{'range':range,'unit':config.tick.type,'method':'round'});
		ticks.push({
			'value': d.toValue(),
			'label': strftime(config.tick.format||'%I',new Date(d.toValue()))
		});
	}
