...
export function getTimeTicks(config){
	...
	let mn = roundDate(config.min,{'range':range,'unit':config.tickType,'method':'floor'});
	let mx = roundDate(config.max,{'range':range,'unit':config.tickType,'method':'ceil'});

	let vmx = mx.plus(t_inc.times(0.2)).toValue();
	for(let v = mn, i = 0; v.toValue() <= vmx; v = v.plus(t_inc), i++){
		let d = roundDate(v.toValue(),{'range':range,'unit':config.tickType,'method':'round'});
		console.log(i,d.toValue());
		ticks.push({
			'value': d.toValue(),
			'label': strftime(config.tickFormat||'%I',new Date(d.toValue()))
		});
	}
