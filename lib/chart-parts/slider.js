...
export function SliderContent(opts){
	...
	if(typeof opts.slider.columns!=="object"){
		...
		fields = fields.concat(opts.slider.columns);
	}
	...
	html += ',"width":"' + opts.slider.width + '"';
	html += ',"defaultbg":' + JSON.stringify(opts.bg);
	...
	html += ',"tooltip": ' + JSON.stringify(opts.tooltip);
	html += ',"columns":'+JSON.stringify(opts.slider.columns);
	html += ',"compresseddata":' + JSON.stringify(tempareas);
	html += ',"fields":' + JSON.stringify(fields);
