...
export function SliderContent(opts){
	...
	if(typeof opts.tooltip!=="string"){
		...
		// Process any `multiply()` filter
		let match = opts.tooltip.match(/multiply\((.*?)\)/);
		if(match){
			// Check which data fields we have
			let row = opts.data[Object.keys(opts.data)[0]];
			for(let col in row){
				if(match[1].match(new RegExp('(^|\\W)'+col+'(\\W|$)'))){
					fields.push(col);
				}
			}
		}
