...
export function Legend(config){
	...
	this.inner = function(method){
		...
		if(config.legend && config.legend.items){
			...
			pos = this.getPosition();
			if(config.legend.continuous){
				steps = (typeof config.legend.steps==="number" ? Math.round(config.legend.steps) : 10);
				// Build our colour scale
				gradient = '';
				for(i = 0; i < steps; i++) gradient += (gradient ? ', ':'')+cs(i/steps)+' '+(100*i/steps).toFixed(1)+'%';
				legend += '<div class="oi-legend-scale" style="background:linear-gradient(to '+(pos.left || pos.right ? 'top':'right')+', '+gradient+');"></div>'
				font = getFontFamily();
			}
			...
			for(i = 0; i < config.legend.items.length; i++){
				...
				if(method=="svg"){
					...
					if(config.legend.continuous){
						legenditem += '<div class="oi-legend-item" style="'+(pos.left || pos.right ? 'top':'right')+':'+(100*(i/(config.legend.items.length-1))).toFixed(1)+'%;">';
						legenditem += '<span class="oi-legend-label">' + label + '</span>';
						legenditem += '</div>';
					}else{
						legenditem += '<div class="oi-legend-item" data-series="'+(i+1)+'">';
