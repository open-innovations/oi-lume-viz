...
export function SVGMap(opts){
	...
	layerlist.push({
		...
		'style': function(feature,el){
			...
			if(row.colour === undefined) row.colour = "#aaaaaa";
...
function BasicMap(config,attr){
	...
	// Add the SVG
	this.svg = svgEl('svg');
	setAttr(this.svg,{'class':'map-inner','xmlns':'http://www.w3.org/2000/svg','version':'1.1','width':this.w,'height':this.h,'viewBox':'-180 0 360 180','overflow':'hidden','style':'max-width:100%;max-height:100%;height:auto;background:'+(attr.background||"white"),'preserveAspectRatio':'xMidYMin meet'});
